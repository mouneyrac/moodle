...
class core_auth_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function linkingaccountlink($provider, $username) {
        $pagetitle = html_writer::tag('h2',
        ...
        $htmlalreadyregistered = $this->loginforms($username,
                array('accounttolink' => $provider->shortname));
     ...
     * @return string
     */
    public function forceaccountlink($provider, $username) {
        $pagetitle = html_writer::tag('h2',
     ...
     * @return string
     */
    private function loginforms($username, $additionalparams) {
        global $CFG;
        ...
        foreach ($auths as $auth) {
            ...
            if (file_exists($CFG->dirroot . '/auth/' . $auth . '/renderer.php')) {
                ...
                $loginformshtml .= $authrenderer->loginform($username, $additionalparams);
            }
        }
