...
/**
 * Add course module - no capability check
 *
 * @param mixed $moduleinfo
 * @param mixed $course
 * @param mixed $mform - this is required by an  hack to deal with files during MODULENAME_add_instance() -
 *                     we need to get rid of this hack. Core lib function should not deal with mform.
 * @access public
 * @return void
 */
function add_module($moduleinfo, $course, $mform = null) {
    global $DB, $CFG;
...
/**
 * Set some module data default value before creating/updating a module.
 *
...
function can_add_module($course, $modulename, $section) {
    global $DB;
...
function update_module($cm, $moduleinfo, $course, $mform = null) {
    global $DB, $CFG;
...
function include_once_module($module) {
    global $CFG;
 ...
 *      - create the module
 *      - executing all post necessary post actions
 *
 * @param object $module
 ...
 *               $moduleinfo->introeditor => array('text' => '', 'format' => FORMAT_XXX, 'idemid' => $draftid_editor)
...
function create_module($moduleinfo) {
    ...
    list($module, $context, $cw) = can_add_module($course, $moduleinfo->modulename, $moduleinfo->section);

    // Load module library.
    include_once_module($module);

    // Add the module.
    $moduleinfo->module = $module->id;
    $moduleinfo = add_module($moduleinfo, $course, null);
 ...
 *               $moduleinfo->gradecat
 *               $moduleinfo->advancedgradingmethod_ + $areaname ???
 * @return object the moduleinfo
 */
function update_moduleinfo($moduleinfo) {
    global $DB;
    ...
    list($cm, $context, $module, $data, $cw) = can_update_module($cm);

    // Load module library
    include_once_module($module);

    // Retrieve few information needed by update_module
    ...
    // Update the module
    list($cm, $moduleinfo) = update_module($cm, $moduleinfo, $course, null);
