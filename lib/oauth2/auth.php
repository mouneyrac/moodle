...
abstract class auth_plugin_oauth2 extends auth_plugin_base {
    ...
    function loginpage_hook() {
        ...
        $authorizationcode = optional_param('oauth2code', '', PARAM_TEXT);
        ...
        if (!empty($authorizationcode) and ($authprovider == $this->shortname)) {
            ...
            if ($this->oauth2client->upgrade_token($authorizationcode)) {
                ...
                if (empty($this->oauth2client->oauth2user->email) or $this->oauth2client->oauth2user->email != clean_param($this->oauth2client->oauth2user->email, PARAM_EMAIL)) {
                ...
                if ($profilelinking) {
                    ...
                    // Link the account
                    // TODO: if ever we come to want to support a provideruserid that
                ...
                if ($linkedaccount = $this->get_linked_account($this->oauth2client->oauth2user->id)) {
                    $user = $DB->get_record('user', array('id' => $linkedaccount->userid, 'deleted' => 0, 'mnethostid' => $CFG->mnet_localhost_id));
                } else {
                    //get the user - don't bother with auth = oauth2xxxx because
                    //authenticate_user_login() will fail it if it's not 'oauth2xxxx'
                    $user = $DB->get_record('user', array('email' => $this->oauth2client->oauth2user->email, 'deleted' => 0, 'mnethostid' => $CFG->mnet_localhost_id));
                }
                ...
                if (empty($user)) {
                    ...
                    $this->link_account($this->oauth2client->oauth2user->id, $createduser->id);
                ...
                add_to_log(SITEID, 'auth_' . $this->shortname, '', '', $username . '/' . $this->oauth2client->oauth2user->email . '/' . $userid);
                $user = authenticate_user_login($username, null, $this->shortname);
                if ($user) {

                    //set a cookie to remember what auth provider was selected
                    setcookie('MOODLEOAUTH2_' . $CFG->sessioncookie, $authprovider, time() + (DAYSECS * 60), $CFG->sessioncookiepath, $CFG->sessioncookiedomain, $CFG->cookiesecure, $CFG->cookiehttponly);
                    ...
                    // Authentication fail - we still continue the redirection (at worst the user will go back to login page)
                }
    ...
    function config_form($config, $err, $user_fields) {
        ...
        // Client ID.
        $clientidlabel = html_writer::tag('label', get_string('auth_clientid_key', 'auth_' . $this->shortname), array('class' => 'clientidlabel'));
        ...
        $clientid = html_writer::tag('div', html_writer::tag('span', $clientidlabel, array('class' => 'oauth2settingrow1')) .
                        ...
                        html_writer::tag('span', get_string('auth_clientid', 'auth_' . $this->shortname), array('class' => 'oauth2settingrow3')), array('class' => 'oauth2setting'));

        // Client secret.
        $clientsecretlabel = html_writer::tag('label', get_string('auth_clientsecret_key', 'auth_' . $this->shortname), array('class' => 'clientsecretlabel'));
        ...
        $clientsecret = html_writer::tag('div', html_writer::tag('span', $clientsecretlabel, array('class' => 'oauth2settingrow1')) .
                        ...
                        html_writer::tag('span', get_string('auth_clientsecret', 'auth_' . $this->shortname), array('class' => 'oauth2settingrow3')), array('class' => 'oauth2setting'));
        ...
        $userprefix = html_writer::tag('div', html_writer::tag('span', $userprefixlabel, array('class' => 'oauth2settingrow1')) .
                        ...
                        html_writer::tag('span', get_string('auth_userprefix_help', 'auth', $authprefix), array('class' => 'oauth2settingrow3')), array('class' => 'oauth2setting'));
        ...
        // TODO: clean up to make it use httml_writer (keep it consistent over all auth plugins)
    ...
    private function link_account($provideruserid, $userid = null) {
        ...
        $linkedaccount = $DB->get_record('user_auths', array('component' => 'auth_' . $this->shortname, 'provideruserid' => $provideruserid));
        if ($linkedaccount) {
            ...
            if ($linkedaccount->userid != $userid) {
                ...
                $DB->update_record('user_auths', $linkedaccount);
            }
        } else {

            $linkedaccount = $DB->get_record('user_auths', array('userid' => $userid, 'component' => 'auth_' . $this->shortname,));
            if ($linkedaccount) {
                ...
                $DB->insert_record('user_auths', $linkedaccount);
            }
        }
    ...
    private function get_linked_account($provideruserid) {
        global $DB;
        return $DB->get_record('user_auths',
