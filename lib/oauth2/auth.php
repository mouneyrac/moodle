...
abstract class auth_plugin_oauth2 extends auth_plugin_base {
    ...
    public function loginpage_hook() {
        ...
        if (!empty($authorizationcode) and ($authprovider == $this->shortname)) {
            ...
            if ($this->oauth2client->upgrade_token($authorizationcode)) {
                ...
                $usersql = 'SELECT user.* FROM {user_idps} idps, {user} user
                            WHERE idps.provideruserid = :provideruserid AND idps.component = :component
                                  AND user.id = idps.userid AND user.deleted = :deleted
                                  AND user.confirmed = :confirmed AND user.mnethostid = :mnethostid';
                ...
                $authenticateduser = authenticate_user_login($username, null, $this->shortname);
    ...
    private function create_user() {
        ...
        // Check that the auth plugin allow user creation (hack attempt).
