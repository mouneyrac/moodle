...
class core_test_util_testcase extends advanced_testcase {
    ...
    public function test_reset_dataroot() {
        ...
        $mockutil = new mock_util();

        $mockutil->installjsonfile();

        // Check that the json file has been created and contain the init data root filedir file.
        $this->assertEquals(true, file_exists($mockutil::get_dataroot() . '/' . $mockutil::get_originaldatafilesjson()));

        // Create a file in dataroot (Simulate creation of a file when running a test).
        $file = $mockutil::get_dataroot() . '/filedir/testfile.txt';
        ...
        $fi = new FilesystemIterator($mockutil::get_dataroot() . '/filedir/', FilesystemIterator::SKIP_DOTS);
        $this->assertEquals(2, iterator_count($fi));

        $mockutil::reset_dataroot();

        // Check we have only one file in the dataroot.
        // Check we have two files in the dataroot.
        $fi = new FilesystemIterator($mockutil::get_dataroot() . '/filedir/', FilesystemIterator::SKIP_DOTS);
        ...
        $mockutil->dropdataroot();

        // Check that there is no json file.
        // Check that the json file has been created and contain the init data root filedir file.
        $this->assertEquals(false, file_exists($mockutil::get_dataroot() . '/' . $mockutil->get_originaldatafilesjson()));

        // Check there is not filedir directory.
        $this->assertEquals(0, file_exists($mockutil::get_dataroot() . '/filedir'));
    }
}
...
class mock_util extends testing_util {
    ...
    function __construct(){
        ...
        $mockdataroot = $CFG->phpunit_dataroot . '/mockdataroot';

        // Create a new dataroot.
        mkdir($mockdataroot);
        // Create a new dataroot.
        mkdir($mockdataroot . '/filedir');
        // create the framework folder.
        mkdir($mockdataroot . '/mock');

        self::set_dataroot($mockdataroot);
