...
MANAGER.prototype = {
    ...
    /**
     * An associative object of the dragdelegations
     * @property dragdelegations
     * @type Array of Y.DD.Delegate
     */
    dragdelegations : {},
    ...
    initializer : function() {
        ...
        for (i in regionnames) {
            ...
            this.dragdelegations[regionname] = dragdelegation;

            region.remove_block_move_icons();
        }
        Y.log('Initialisation of drag and drop for blocks complete.', 'info');

        this.update_block_header_ajax_support();

        // Update the ajax block header when the window is resized.
        Y.one('window').on('resize', this.update_block_header_ajax_support, this);
    },

    /**
     * Add the ajax handler + move cursor to the block headers when the block is not full window width.
     * Otherwise remove them.
     * @method update_block_header_ajax_support
     */
    update_block_header_ajax_support : function() {
        var regionname;
        for (regionname in this.regionobjects) {
            // Detect if the block takes most of the screen width.
            // If yes, then scrolling will conflict with drag&drop, we need to remove the ajax-header drag&drop.
            header = this.regionobjects[regionname].get_droptarget().one('.'+CSS.BLOCK + ' .'+CSS.HEADER);
            blockwidth = header.get('offsetWidth');
            windowdwidth = header.get('winWidth');
            if (windowdwidth * 0.80 < blockwidth) {
                this.dragdelegations[regionname].dd.removeHandle('.'+CSS.HEADER);

            } else {
                this.dragdelegations[regionname].dd.addHandle('.'+CSS.HEADER);
            }
            this.regionobjects[regionname].update_block_header_cursor();
        }
...
BLOCKREGION.prototype = {
    ...
    remove_block_move_icons : function() {
        ...
        this.update_block_header_cursor();
    },

    /**
     * Change the cursor to a move icon when over the header.
     * @method update_block_header_cursor
     */
    update_block_header_cursor : function() {
        this.get('node').all('.'+CSS.BLOCK + ' .'+CSS.HEADER).each(function(blockheader){
            // Detect if the block takes most of the screen width.
            // If yes, then scrolling will conflict with drag&drop, we need to remove the ajax-header drag&drop.
            blockwidth = blockheader.get('offsetWidth');
            windowdwidth = blockheader.get('winWidth');
            if (windowdwidth * 0.80 > blockwidth) {
                blockheader.setStyle('cursor', 'move');
            } else {
                blockheader.setStyle('cursor', 'default');
            }
        });
    },
