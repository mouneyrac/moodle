...
if (isloggedin() and !isguestuser()) {
    ...
    echo $OUTPUT->box_end();
} else if(optional_param('createorlinkrequest', false, PARAM_BOOL)) {
    ...
    if (get_config('auth/' . $provider->shortname, 'createuser')) {
         echo $output->linkingaccountlink($provider, $frm->username);
    } else {
         echo $output->forceaccountlink($provider, $frm->username);
    }

    // Browse all enable auth and look for renderer, then display them
