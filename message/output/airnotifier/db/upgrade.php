...
function xmldb_message_airnotifier_upgrade($oldversion) {
    ...
    if ($oldversion < 2012070500.03) {
        ...
        upgrade_plugin_savepoint(true, 2012070500.03);
    }


    if ($oldversion < 2012070500.04) {

        // Define field enable to be added to airnotifier_user_devices
        $table = new xmldb_table('airnotifier_user_devices');
        $field = new xmldb_field('enable', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '1', 'userid');

        // Conditionally launch add field enable
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // airnotifier savepoint reached
        upgrade_plugin_savepoint(true, 2012070500.04, 'message', 'airnotifier');
    }
